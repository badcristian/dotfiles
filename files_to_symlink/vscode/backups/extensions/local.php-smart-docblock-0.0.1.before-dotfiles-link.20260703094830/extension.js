...
function activate(context) {
  context.subscriptions.push(vscode.commands.registerTextEditorCommand('php-smart-docblock.generate', async editor => {
    ...
    if (!found) {
      if (await goToDefinition(editor)) {
        return;
      }

      await vscode.commands.executeCommand('php-docblocker.trigger');
      return;
    }

    const currentLine = editor.document.lineAt(found.line).text;
    const indent = (currentLine.match(/^\s*/) || [''])[0];
    const snippet = new vscode.SnippetString(indent + '/** @var \${1:TYPE_NAME} ' + found.name + ' */\n');
